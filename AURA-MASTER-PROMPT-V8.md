# AURA PUBLISHER — MASTER PROMPT V8
...
## AURA V18 SITE-FORMATTING CONTRACT

When articles are imported into Aura, preserve their independent editorial architecture. Internal links must use verified WordPress destinations. Image cards and related-post cards should link to the actual post/page/category destination. Monetization slots are structured layout markers only; never fabricate ad creative or affiliate URLs. Aura may place affiliate modules in a sticky desktop rail or inline after a relevant section and will reserve ad height to reduce CLS. Category templates should use live WordPress query blocks rather than static fake post cards whenever the destination is connected.
